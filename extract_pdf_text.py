...
def chunk_text(text: str, max_chars: int = 250, overlap: int = 50) -> List[str]:
    """
...
def store_chunks_in_chromadb(chunks_data: List[Dict], collection_name: str = "slide_chunks", deck_name: str = None):
    """
    ...
    if deck_name:
        try:
            ...
            if ids_to_delete:
                ...
                print(f"✓ Removed {len(ids_to_delete)} existing chunks for deck '{deck_name}'")
        except Exception as e:
    ...
    for i, chunk_data in enumerate(chunks_data):
        ...
        ids.append(f"{chunk_data['metadata']['deck_name']}_slide_{chunk_data['metadata']['slide_number']}_chunk_{i}")
    ...
    print(f"✓ Stored {len(chunks_data)} chunks in ChromaDB collection '{collection_name}'")


def process_pdf_with_chunking(pdf_path: Path, store_in_db: bool = False) -> List[Dict]:
    """
    Process a PDF file, extract text, and treat each page as a single chunk.
                ...
                try:
                    ...
                    chunk_data = {
                        ...
                        'metadata': {
                            ...
                            'chunk_index': 0,  # Always 0 since each page is one chunk
    ...
    print(f"✓ Created {len(chunks_data)} chunks from {pdf_path.name} (1 chunk per page)")
...
@click.command()
@click.option('-d', '--deck', help='Extract text from a specific deck (without .pdf extension)')
@click.option('-a', '--all', is_flag=True, help='Extract text from all decks in the folder')
@click.option('--chromadb', is_flag=True, help='Store chunks in ChromaDB')
def main(deck, all, chromadb):
    """PDF Text Extraction Tool for slide decks."""
    print("PDF Text Extraction Tool")
    print("=" * 30)
    
    if deck and all:
        print("Error: Cannot use both -d and -a options together")
        return
    
    if not deck and not all:
        print("Error: Please specify either -d <deck_name> or -a")
        print("Usage:")
        print("  python extract_pdf_text.py -d <deck_name> [--chromadb]  # Extract from specific deck")
        print("  python extract_pdf_text.py -a [--chromadb]             # Extract from all decks")
        print("  --chromadb flag stores chunks in ChromaDB")
        return
    ...
    if deck:
        # Process single deck with chunking
        deck_name = deck
        if deck_name.endswith('.pdf'):
            deck_name = deck_name[:-4]
        ...
        # Remove existing chunk files for this deck
        output_dir = Path("extracted_text")
        if output_dir.exists():
            existing_chunk_files = list(output_dir.glob(f"{deck_name}_chunks.txt"))
            for file_to_remove in existing_chunk_files:
                file_to_remove.unlink()
                print(f"✓ Removed existing chunk file: {file_to_remove}")
        
        chunks_data = process_pdf_with_chunking(deck_path, store_in_db=chromadb)
        all_chunks_data.extend(chunks_data)
        
        if not chromadb:
            # Save chunks to text file for inspection
            output_dir.mkdir(exist_ok=True)
            output_file = output_dir / f"{deck_name}_chunks.txt"
            
            with open(output_file, 'w', encoding='utf-8') as f:
                for i, chunk_data in enumerate(chunks_data):
                    f.write(f"=== Chunk {i+1} ===\n")
                    f.write(f"Deck: {chunk_data['metadata']['deck_name']}\n")
                    f.write(f"Slide: {chunk_data['metadata']['slide_number']}\n")
                    f.write(f"Title: {chunk_data['metadata']['slide_title']}\n")
                    f.write(f"Text: {chunk_data['text']}\n\n")
        ...
        # Remove existing chunk files when processing all decks
        output_dir = Path("extracted_text")
        if output_dir.exists():
            existing_chunk_files = list(output_dir.glob("*_chunks.txt")) + list(output_dir.glob("all_chunks.txt"))
            for file_to_remove in existing_chunk_files:
                file_to_remove.unlink()
                print(f"✓ Removed existing chunk file: {file_to_remove}")
        
        for pdf_file in sorted(pdf_files):
            chunks_data = process_pdf_with_chunking(pdf_file, store_in_db=False)
            all_chunks_data.extend(chunks_data)
        
        if chromadb and all_chunks_data:
            ...
            store_chunks_in_chromadb(all_chunks_data)
        
        if not chromadb:
            # Save all chunks to a combined file
            output_dir.mkdir(exist_ok=True)
            output_file = output_dir / "all_chunks.txt"
            ...
            print(f"✓ Saved {len(all_chunks_data)} chunks to: {output_file}")
